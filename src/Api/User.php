...
class User extends Base
{
     ...
     */
    private function verifyUserInfo($accid, $name, array $props = [], $icon, $token, $sign, $email, $birth, $mobile, $gender, $ex)
    {
     ...
     * @return mixed
     */
    public function create($accid, $name, array $props = [], $icon = '', $token = '', $sign = '', $email = '', $birth = '', $mobile = '', $gender = 0, $ex = '')
    {
        $this->verifyUserInfo($accid, $name, $props, $icon, $token, $sign, $email, $birth, $mobile, $gender, $ex);
    ...
    {
        $this->verifyUserInfo($accid, '', $props, '', $token, '', '', '', '', 0, '');

        return $this->sendRequest('user/update.action', [
            'accid' => $accid,
            'props' => json_encode($props),
            'token' => $token,
        ]);
    ...
    public function block($accid, $kick = false)
    {
        ...
        return $this->sendRequest('user/block.action', [
            'accid' => $accid,
            'needkick' => $kick,
        ]);
    }
    ...
    public function unblock($accid)
    {
        ...
        return $this->sendRequest('user/unblock.action', [
            'accid' => $accid,
        ]);
    }
    ...
    {
        $this->verifyUserInfo($accid, $name, [], $icon, '', $sign, $email, $birth, $mobile, $gender, $ex);

        return $this->sendRequest('user/updateUinfo.action', [
            'accid' => $accid,
            'name' => $name,
            'icon' => $icon,
            'sign' => $sign,
